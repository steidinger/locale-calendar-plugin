...
public class DisjunctiveConditionGroupTest {
    @Test
    public void groupShouldMatchIfAllConditionsMatch() {
        ConditionGroup group = new DisjunctiveConditionGroup();
    ...
    @Test
    public void groupShouldMatchIfOneConditionDoesNotMatch() {
        ConditionGroup group = new DisjunctiveConditionGroup();
        group.conditions.add(new TitleDoesContainText("test"));
        group.conditions.add(new TitleDoesContainText("other"));

        assertThat(group.matches(entry("just a test")), equalTo(true));
    }

    @Test
    public void groupShouldNotMatchIfNoConditionDoesMatch() {
        ConditionGroup group = new DisjunctiveConditionGroup();
        group.conditions.add(new TitleDoesContainText("test"));
        group.conditions.add(new TitleDoesContainText("other"));

        assertThat(group.matches(entry("something else")), equalTo(false));
    }
